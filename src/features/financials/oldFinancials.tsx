...
const mapToUIFormat = (sourceData: {
  ...
  const assumptions: RevenueAssumption[] = sourceData.financialRevenueStreams
    .filter(
      (stream) => stream.assumptions !== null && stream.assumptions !== ""
    )
    .map((stream) => ({
      id: stream.id,
  ...
  const competitorPrices: CompetitorPrice[] =
    sourceData.financialPricingStrategies
  ...
  const breakEvenProjection = sourceData.financialProjections.find(
    (p) => p.break_even
  );
  ...
  if (breakEvenProjection && breakEvenProjection.break_even) {
    try {
      const breakEvenData =
        typeof breakEvenProjection.break_even === "string"
  ...
  const avgUnitPrice = sourceData.financialRevenueStreams
    ...
    .reduce(
      (sum, stream, index, array) =>
        sum + (stream.unit_price || 0) / array.length,
      0
    );
...
const getBreakevenTimeframe = (data: FinancialData): string => {
  ...
  for (let i = 0; i < sortedForecasts.length; i++) {
    ...
    const unitsSold =
      data.breakeven.unitSellingPrice > 0
...
export const FinancialProjections: React.FC = () => {
  ...
  const getItemStatus = (
    ...
    const getCurrentItem = () => {
      ...
      switch (section) {
        ...
        case "pricing.competitorPrices":
          return data.pricing.competitorPrices.find(
            (item) => item.id === itemId
          );
        default:
          return undefined;
    ...
    const getStagedItem = () => {
      ...
      switch (section) {
        case "revenue.forecasts":
          return stagedUIData.revenue.forecasts.find(
            (item) => item.id === itemId
          );
        case "revenue.assumptions":
          return stagedUIData.revenue.assumptions.find(
            (item) => item.id === itemId
          );
        case "costs.fixedCosts":
          return stagedUIData.costs.fixedCosts.find(
            (item) => item.id === itemId
          );
        case "costs.variableCosts":
          return stagedUIData.costs.variableCosts.find(
            (item) => item.id === itemId
          );
        case "pricing.strategies":
          return stagedUIData.pricing.strategies.find(
            (item) => item.id === itemId
          );
        case "pricing.competitorPrices":
          return stagedUIData.pricing.competitorPrices.find(
            (item) => item.id === itemId
          );
        default:
          return undefined;
    ...
    if (currentItem && stagedItem) {
      return JSON.stringify(currentItem) !== JSON.stringify(stagedItem)
        ? "modified"
        : "unchanged";
    }
  ...
  const updateBreakeven = (updates: Partial<BreakevenData>) => {
    ...
    const existingProjection = currentData.financialProjections.find(
      (p) => p.break_even
    );
  ...
  const calculateBreakeven = (data: BreakevenData): BreakevenData => {
    ...
    const breakEvenUnits =
      contributionMarginValue > 0 ? fixedCosts / contributionMarginValue : 0;
