...
import { MarketPersona, Update } from '@/store/types';

// Schema for form validation - same as in AddPersonaForm
...
interface EditPersonaFormProps {
  ...
  onSubmit: (data: Update<'market_personas'>) => Promise<void>;
  onCancel: () => void;
}

export const EditPersonaForm: React.FC<EditPersonaFormProps> = ({ 
  persona, 
  onSubmit, 
  onCancel 
}) => {
  // Initialize form with validation schema and persona values
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      priority: persona.priority as 'primary' | 'secondary' | 'tertiary' | null,
  ...
  // Handle form submission - only update changed fields
  const handleSubmit = async (values: FormValues) => {
    const updates: Update<'market_personas'> = {};
    
    // Only include fields that have changed
    if (values.name !== persona.name) updates.name = values.name;
    if (values.role !== persona.role) updates.role = values.role;
    if (values.demographics !== persona.demographics) updates.demographics = values.demographics;
    
    // For array fields, compare stringified versions for simple comparison
    if (JSON.stringify(values.goals) !== JSON.stringify(persona.goals)) {
      updates.goals = values.goals;
    }
    
    if (JSON.stringify(values.pain_points) !== JSON.stringify(persona.pain_points)) {
      updates.pain_points = values.pain_points;
    }
    
    if (JSON.stringify(values.persona_segments) !== JSON.stringify(persona.persona_segments)) {
      updates.persona_segments = values.persona_segments;
    }
    
    if (values.influence_score !== persona.influence_score) {
      updates.influence_score = values.influence_score;
    }
    
    if (values.priority !== persona.priority) {
      ...
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
        ...
        <AlertDialogFooter className="pt-4">
          ...
          <Button type="submit" disabled={form.formState.isSubmitting}>
            {form.formState.isSubmitting ? 'Updating...' : 'Update Persona'}
          </Button>
        </AlertDialogFooter>
      </form>
    </Form>
